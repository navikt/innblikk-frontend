...
const getFetchResultData = <T extends JsonObject>(value: unknown, itemGuard: (item: unknown) => item is T): T[] => {
 ...
 */
function buildCombinedSessionQuery(websiteId: string, filters: Filters, fields: string[]): string {
...
export async function fetchDashboardDataBatched(
  ...
  console.log(
    `[QueryBatcher] Found ${sessionCharts.length} session charts to batch (${sessionFieldsNeeded.join(', ')})`,
  )
  ...
  if (sessionCharts.length > 1 && sessionFieldsNeeded.length > 0) {
    try {
      ...
      console.log(
        `[QueryBatcher] Batched query processed ${Math.round(batchBytes / 1024 ** 3)} GB for ${sessionCharts.length} charts`,
      )
    } catch (error) {
