...
const getStoredProjectId = (): number | null =>
  typeof window === 'undefined' ? null : parseStoredId(window.localStorage.getItem(LAST_PROJECT_KEY))
const getStoredDashboardId = (): number | null =>
  typeof window === 'undefined' ? null : parseStoredId(window.localStorage.getItem(LAST_DASHBOARD_KEY))
const getStoredCategoryId = (): number | null =>
  typeof window === 'undefined' ? null : parseStoredId(window.localStorage.getItem(LAST_CATEGORY_KEY))
const getStoredWebsiteId = (): string =>
  typeof window === 'undefined' ? '' : (window.localStorage.getItem(LAST_WEBSITE_KEY) ?? '')
...
const AddToDashboardDialog = ({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const nextProjectId =
          storedProjectId && items.some((item) => item.id === storedProjectId) ? storedProjectId : (items[0]?.id ?? 0)
  ...
  const handleSave = async () => {
    ...
    setError(null)
    try {
      const sqlToSave = websiteId ? sqlText.replace(/\{\{\s*website_id\s*\}\}/g, websiteId) : sqlText
