...
const CombinedEntrancesTable = ({
  ...
  const renderName = (row: { name: string; type: 'external' | 'internal' }) => {
    ...
    if (
      selectedWebsite &&
      row.name.toLowerCase().replace(/^www\./, '') === selectedWebsite.domain.toLowerCase().replace(/^www\./, '')
    ) {
                ...
                <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto min-w-0">
                  {showFilter && (
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 w-full sm:w-auto">
                      <span className="text-sm text-[var(--ax-text-default)] whitespace-nowrap">Type trafikkilde</span>
