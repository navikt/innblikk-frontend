...
const AnalysisTable = ({
  ...
  const handleDownloadCSV = () => {
    ...
    const csvRows = [
      headers.join(','),
      ...data.map((item) => {
        return [`"${item.name}"`, metricType === 'proportion' ? `${item.count.toFixed(1)}%` : item.count].join(',')
      }),
    ]
    ...
    downloadCsvFile(
      csvContent,
      `marketing_${title.toLowerCase().replace(/\s+/g, '_')}_${new Date().toISOString().slice(0, 10)}.csv`,
    )
  }
      ...
      return (
        <div className="flex items-center gap-2 max-w-full">
          ...
          <HelpText title="Hva betyr dette?" strategy="fixed">
            Besøk hvor det ikke er registrert noen henvisningskilde. Dette er ofte brukere som skriver inn nettadressen
            direkte, bruker bokmerker, eller kommer fra apper (som e-post eller Teams) som ikke sender data om hvor
    ...
    if (name === '(exit)') {
      return (
        <div className="flex items-center gap-2 max-w-full">
          ...
          <HelpText title="Hva betyr dette?" strategy="fixed">
            Dette viser vanligvis til økter som ble avsluttet uten ny sidevisning, eller data som mangler
            kildeinformasjon ved utgang.
          </HelpText>
        </div>
      )
    }
    ...
    if (selectedWebsite && name === selectedWebsite.domain) {
      return (
        <div className="flex items-center gap-2 max-w-full">
          ...
          <HelpText title="Hva betyr dette?" strategy="fixed">
            Trafikk som ser ut til å komme fra samme domene. Dette skjer ofte ved omdirigeringer, eller hvis
            sporingskoden mistet sesjonsdata mellom to sidevisninger.
          </HelpText>
        </div>
      )
    }
        ...
        <div className="border border-[var(--ax-border-neutral-subtle)] rounded-lg overflow-hidden bg-[var(--ax-bg-default)]">
                ...
                <>
                  ...
                  <ActionMenu>
                    ...
                    <ActionMenu.Content align="end">
                      ...
                      <ActionMenu.Item
                        onClick={() =>
                          openSqlEditorWithContext({ sql: addToDashboardSql, websiteId: selectedWebsite?.id })
                        }
                      >
                        Åpne i SQL-editor
                      </ActionMenu.Item>
          ...
          {totalPages > 1 ? (
            <div className="px-4 py-3">
              <Pagination page={currentPage} onPageChange={setPage} count={totalPages} size="small" />
            </div>
          ) : (
