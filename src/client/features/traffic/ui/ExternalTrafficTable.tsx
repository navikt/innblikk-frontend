...
const ExternalTrafficTable = ({
  title,
  data,
  metricLabel,
  websiteDomain,
  submittedMetricType,
}: ExternalTrafficTableProps) => {
      ...
      return (
        <div className="flex items-center gap-2 max-w-full">
          ...
          <HelpText title="Hva betyr dette?" strategy="fixed">
            Differansen mellom totalen og summen av identifiserte kanaler. Dette kan skyldes filtrering, begrenset
            antall kilder, eller manglende henvisningsdata.
          </HelpText>
        </div>
      )
    }
    ...
    if (normalizedDomain && normalizedName === normalizedDomain) {
      return (
        <div className="flex items-center gap-2 max-w-full">
          ...
          <HelpText title="Hva betyr dette?" strategy="fixed">
            Besøkende som kom fra andre sider på samme nettsted. For eksempel brukere som klikket på en lenke fra
            forsiden eller en annen underside.
          </HelpText>
        </div>
      )
    }
  ...
  const handleDownloadCSV = () => {
    ...
    const csvRows = [
      headers.join(','),
      ...data.map((item) => {
        return [item.name, formatCsvValue(item.count, submittedMetricType)].join(',')
      }),
    ]
                ...
                <Table.Header>
                  <Table.Row>
                    <Table.HeaderCell
                      ...
                      style={{ width: '6.75rem', minWidth: '6.75rem' }}
                    >
                      {metricLabel}
                    </Table.HeaderCell>
