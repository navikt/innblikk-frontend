...
const TrafficTable = ({
  ...
  const handleDownloadCSV = () => {
    ...
    const headers = showCompare ? ['URL-sti', metricLabel, 'Forrige', 'Endring'] : ['URL-sti', metricLabel]
    const csvRows = [
      headers.join(','),
      ...data.map((item) => {
        const baseRow = [item.name, formatMetricValue(item.count, submittedMetricType)]
        if (showCompare) {
                ...
                <Table.Header>
                  <Table.Row>
                    <Table.HeaderCell
                      ...
                      style={{ width: valueColWidth, minWidth: valueColWidth }}
                    >
                      {metricLabel}
                    </Table.HeaderCell>
                    {showCompare && (
