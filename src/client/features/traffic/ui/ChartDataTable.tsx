...
const ChartDataTable = (props: ChartDataTableProps) => {
  ...
  const shouldShowCompareColumns = Boolean(
    submittedComparePreviousPeriod && previousData.length && submittedDateRange && submittedPreviousDateRange,
  )
  ...
  const filteredData = data.filter((item) => formatTime(item.time).includes(search))
    ...
    const csvRows = [
      ...
      ...data.map((item) => {
        ...
        if (shouldShowCompareColumns) {
          ...
          baseRow.push(
            hasPreviousValue ? formatMetricDeltaUtil(currentValue - previousValue, submittedMetricType) : '-',
          )
        }
        return baseRow.join(',')
      }),
            ...
            <Table size="small" className="table-fixed w-full">
              <Table.Header>
                <Table.Row>
                  ...
                  {shouldShowCompareColumns && <Table.HeaderCell align="right">Forrige</Table.HeaderCell>}
                  {shouldShowCompareColumns && <Table.HeaderCell align="right">Endring</Table.HeaderCell>}
                </Table.Row>
              </Table.Header>
              <Table.Body>
                {paginatedData.map((item, index) => {
                  ...
                  return (
                    <Table.Row key={index}>
                      <Table.DataCell>{formatTime(item.time)}</Table.DataCell>
