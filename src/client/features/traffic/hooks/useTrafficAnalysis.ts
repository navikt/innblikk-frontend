...
export const useTrafficAnalysis = () => {
  ...
  const [submittedMetricType, setSubmittedMetricType] = useState<string>(() =>
  ...
  const getCountByQueryParams = useCallback(
  ...
  const cookieBadge = useMemo(() => {
    ...
    return getCookieBadge(usesCookies, cookieStartDate, currentDateRange.startDate, currentDateRange.endDate)
  ...
  const fetchSeriesData = useCallback(async () => {
    ...
    try {
      ...
      if (comparePreviousPeriod && previousDateRange) {
        ...
        setPreviousSeriesTotalCount(
          typeof previousSeriesResult.totalCount === 'number' ? previousSeriesResult.totalCount : undefined,
        )
      }
  ...
  const fetchTrafficBreakdownHandler = useCallback(
    async (
      startDate: Date,
      endDate: Date,
  ...
  const fetchExternalReferrersHandler = useCallback(
    async (
      startDate: Date,
      endDate: Date,
      options?: { urlPaths?: string[]; pathOperator?: string; metricType?: string },
  ...
  useEffect(() => {
    ...
    hasFetchedBreakdown,
    fetchPageMetricsHandler,
    fetchExternalReferrersHandler,
    fetchTrafficBreakdownHandler,
  ])
  ...
  const chartData = useMemo(() => {
    ...
    const yMax = submittedMetricType === 'proportion' ? Math.max(dataMax * 1.1, 1) : dataMax * 1.1
  ...
  const downloadCSV = () => {
    ...
    const csvRows = [
      ...
      ...seriesData.map((item: SeriesPoint) => {
        ...
        return [timeStr, submittedMetricType === 'proportion' ? `${(item.count * 100).toFixed(1)}%` : item.count].join(
          ',',
        )
      }),
    ]
    ...
    downloadCsvFile(
      csvContent,
      `traffic_${selectedWebsite?.name || 'data'}_${new Date().toISOString().slice(0, 10)}.csv`,
    )
  }
  ...
  const { entrances, exits } = useMemo(() => {
    ...
    const normalizePathValue = (value: string) => (value !== '/' && value.endsWith('/') ? value.slice(0, -1) : value)
    ...
    const exitsList = breakdownData.exits
      .map((e) => ({
        name: e.name,
  ...
  const externalReferrers = useMemo(() => {
    return externalReferrerData
      ...
      .map((item) => ({
        name: item.name,
        count: submittedMetricType === 'proportion' ? Number(item.count) / 100 : Number(item.count),
      }))
  ...
  const comparisonSummary = useMemo(() => {
    ...
    const previousValue = getComparablePeriodValue(
      processedPreviousSeriesData,
      submittedMetricType,
