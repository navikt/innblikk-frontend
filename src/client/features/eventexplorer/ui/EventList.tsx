...
const EventList = ({ events, eventsQueryStats, websiteName, selectedWebsiteId, onSelectEvent }: EventListProps) => {
  ...
  const handleDownloadCsv = () => {
    ...
    const csvRows = [headers.join(','), ...filteredEvents.map((event) => [`"${event.name}"`, event.count].join(','))]
            ...
            <>
              ...
              <ActionMenu>
                ...
                <ActionMenu.Content align="end">
                  ...
                  <ActionMenu.Item
                    onClick={() =>
                      openSqlEditorWithContext({ sql: getEventListSqlTemplate(), websiteId: selectedWebsiteId })
                    }
                  >
                    Åpne i SQL-editor
                  </ActionMenu.Item>
          ...
          <Table.Body>
            {filteredEvents.map((event) => (
              <Table.Row key={event.name}>
                ...
                <Table.DataCell>
                  <Button size="xsmall" variant="secondary" onClick={() => onSelectEvent(event.name)}>
                    Utforsk
                  </Button>
                </Table.DataCell>
              </Table.Row>
