  ...
  const handleDownloadCsv = () => {
    ...
    const csvRows = [
      ...
      ...filteredSeriesData.map((item) => {
        return [new Date(item.time).toLocaleDateString('nb-NO'), item.count].join(',')
      }),
    ]
        ...
        <div className="flex items-center gap-1">
          ...
          <ActionMenu>
            ...
            <ActionMenu.Content align="end">
              <ActionMenu.Item onClick={() => setShowAddToDashboardDialog(true)}>Legg til i dashboard</ActionMenu.Item>
              <ActionMenu.Item onClick={() => setShowTransferToMetabaseDialog(true)}>
                Overfør til Metabase
              </ActionMenu.Item>
              <ActionMenu.Item
                onClick={() => openSqlEditorWithContext({ sql: getEventSeriesSqlTemplate(selectedEvent) })}
              >
                Åpne i SQL-editor
              </ActionMenu.Item>
