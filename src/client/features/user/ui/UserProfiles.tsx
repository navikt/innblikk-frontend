...
const UserProfiles = () => {
  ...
  const downloadProfilesCsv = (filteredUsers: UserProfile[]) => {
    ...
    link.setAttribute(
      'download',
      `brukerprofiler_${selectedWebsite?.name || 'data'}_${new Date().toISOString().slice(0, 10)}.csv`,
    )
      ...
      filters={
        <>
          ...
          <div className="flex items-end pb-[2px]">
            <Button className="w-full sm:w-auto" size="small" onClick={handleSearchClick} disabled={!selectedWebsite}>
              Vis enkeltbrukere
            </Button>
          </div>
        </>
    ...
    >
      ...
      {!loading && selectedWebsite && users.length === 0 && (
        <Alert variant="info" className="mb-4">
          <>
            Ingen brukere funnet. <Link href="/diagnose">Sjekk diagnostikk</Link> for å se om data blir samlet inn.
          </>
        </Alert>
      )}
        ...
        (() => {
          ...
          const filteredUsers = users.filter(
            (user) =>
              ...
              (user.sessionIds &&
                user.sessionIds.some((id: string) => id.toLowerCase().includes(searchQuery.toLowerCase()))) ||
                            ...
                            <ActionMenu.Item
                              onClick={() =>
                                openSqlEditorWithContext({
                                  sql: getUserProfilesSqlTemplate(),
                                  websiteId: selectedWebsite?.id,
                                })
                              }
                            >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4 bg-[var(--ax-bg-neutral-soft)] p-4 rounded-lg">
                ...
                {selectedSession.sessionIds &&
                  selectedSession.sessionIds.length > 0 &&
                  selectedSession.idType === 'cookie' && (
                    <div className="col-span-2">
                      <Heading level="3" size="xsmall" className="mb-1 text-gray-500">
              ...
              <div>
                ...
                {!activityLoading && activityData.length > 0 && (
                  <div className="bg-[var(--ax-bg-neutral-soft)] p-4 rounded-lg mb-6 flex flex-wrap gap-8 border border-gray-100">
                    <div>
                      <span className="text-[var(--ax-text-default)] block text-sm font-medium mb-1">
                        Sidevisninger
                      </span>
                    ...
                    <div>
                      ...
                      <span className="font-bold text-2xl text-[var(--ax-text-default)]">{activityData.length}</span>
                    </div>
                  </div>
                )}
                      ...
                      <div key={index} className="mb-8 ml-8 relative">
                        ...
                        <div className="flex flex-col gap-1">
                          <span className="text-sm text-gray-500">
                            {new Date(item.createdAt).toLocaleTimeString('no-NO', {
                              timeZone: 'Europe/Oslo',
                              hour: '2-digit',
                              minute: '2-digit',
                            })}
                          ...
                          <p className="text-base text-[var(--ax-text-default)]">{item.title || item.url}</p>
                          {item.url && (
                            <code
