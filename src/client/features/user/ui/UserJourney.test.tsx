...
import UserJourney from './UserJourney.tsx'

const FAKE_WEBSITES = [{ id: 'site-1', name: 'Nav.no - prod', domain: 'nav.no', createdAt: '2023-01-01T00:00:00Z' }]

function renderPage() {
  return render(
    <MemoryRouter initialEntries={['/brukerreiser']}>
      <UserJourney />
    </MemoryRouter>,
  )
}

describe('UserJourney page', () => {
  beforeEach(() => {
    localStorage.clear()
    vi.stubGlobal(
      'fetch',
  ...
  it('renders the page title', async () => {
    renderPage()
    expect(screen.getByRole('heading', { name: /navigasjonsflyt/i })).toBeInTheDocument()
