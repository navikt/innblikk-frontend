...
export function useGrafdeling() {
    ...
    (includeAverage: boolean = false) => {
      ...
      if (keys.length === 3) {
        ...
        data.forEach((row: any) => {
          ...
          seriesMap
            .get(seriesValue)!
            .push({ x, y: yValue, xAxisCalloutData: String(xValue), yAxisCalloutData: String(yValue) })
        })
        ...
        if (includeAverage) {
          ...
          lineChartData.forEach((series) =>
            series.data.forEach((point: any) =>
              allXValues.add(point.x instanceof Date ? point.x.getTime() : Number(point.x)),
            ),
          )
          ...
          lineChartData.push({
            ...
            data: averagePoints,
            color: '#262626',
            lineOptions: { lineBorderWidth: '2', strokeDasharray: '5 5' } as any,
          })
        }
      ...
      const lineChartData: any[] = [
        { legend: yKey, data: chartPoints, color: '#0067C5', lineOptions: { lineBorderWidth: '2' } },
      ]
      if (includeAverage) {
  ...
  const prepareBarChartData = useCallback(() => {
    ...
    const total = data.reduce(
      (sum: number, row: any) =>
        sum + (typeof row[valueKey] === 'number' ? row[valueKey] : parseFloat(row[valueKey]) || 0),
      0,
    )
    ...
    return {
      ...
      legendProps: {
        ...
        styles: {
          ...
          legend: { marginRight: 0, fontSize: '16px' },
        },
      },
    }
  }, [result])
  ...
  useEffect(() => {
    ...
    if (sqlParam) {
      ...
      const hasOverrideInputs = Boolean(
        websiteIdParam || urlPathFromUrl || dateRangeFromUrl || customStartFromUrl || customEndFromUrl,
      )
      ...
      const dateMatch = modifiedSql.match(
        /created_at\s+BETWEEN\s+TIMESTAMP\('([^']+)'[^)]*\)\s+AND\s+TIMESTAMP\('([^']+)'[^)]*\)/i,
      )
      if (dateMatch) {
