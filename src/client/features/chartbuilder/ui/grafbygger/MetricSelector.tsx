...
const MetricSelector = forwardRef(
  (
    ...
    const dropdownOptionNeedsInput = (option: MetricDropdownOption): boolean => {
      ...
      const metricIndex = findMetricIndex(option.metricFunction, option.column, option.alias, option.showInMinutes)
            ...
            <div className="space-y-4 mb-3">
              <div>
                ...
                {selectedDropdownOptions.some((o) => dropdownOptionNeedsInput(o)) && (
                  <div className="mt-2 rounded-md border border-(--ax-border-accent) bg-(--ax-bg-accent-soft) px-3 py-2 text-sm">
                    ...
                    <button type="button" className="underline" onClick={() => setShowActiveMetrics(true)}>
                      Vis aktive valg
                    </button>
                  </div>
                )}
              ...
              {showActiveMetrics &&
                metrics.map((metric, index) => (
                  <div
                    key={index}
                    className={`flex ${shouldShowDetailedView(metric, index) ? 'flex-col' : 'items-center justify-between'} bg-[var(--ax-bg-default)] px-4 py-3 rounded-md border`}
                  >
                      ...
                      <div className="flex items-center gap-2">
                        {isShortcutMetric(metric) ? (
                          <>
                            ...
                            <Button variant="tertiary-neutral" size="small" onClick={() => removeMetric(index)}>
                              Fjern
                            </Button>
                          </>
                        ) : (
                            ...
                            <div className="bg-[var(--ax-bg-neutral-soft)] p-3 rounded border">
                              ...
                              <div className="flex flex-col gap-2">
                                ...
                                {metric.whereColumn === 'event_name' ? (
                                  <div>
                                    {isEventsLoading && (
                                      <div className="text-xs text-[var(--ax-text-subtle)] mb-1">
                                        Laster hendelser...
                                      </div>
                                    )}
                                    ...
                                    <>
                                      {Object.entries(COLUMN_GROUPS).map(([groupKey, group]) => (
                                        <optgroup key={groupKey} label={(group as { label: string }).label}>
                                          {(group as { columns: { value: string; label: string }[] }).columns.map(
                                            (col) => (
                                              <option key={col.value} value={col.value}>
                                                {col.label}
                                              </option>
