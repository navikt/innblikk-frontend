...
const QueryPreview = ({
  ...
  const hasChanges = () => {
    const dateChanged =
      !isEqual(dateRange.from || 0, executedParams.dateRange.from || 0) ||
  ...
  useEffect(() => {
        ...
        (f) =>
          ...
          ((f.interactive === true && f.metabaseParam === true) ||
            (typeof (f as { value?: unknown }).value === 'string' &&
              /\{\{\s*url_sti\s*\}\}/i.test((f as { value?: string }).value || ''))),
      )
    ...
    setHasUrlPathFilter(
      urlPathOptionalPattern.test(sql) || urlPathDirectPattern.test(sql) || hasUrlPathInteractiveFilter,
    )
  ...
  const getProcessedSql = (options?: { preserveMetabasePlaceholders?: boolean }) => {
    ...
    if (hasUrlPathFilter) {
      ...
      if (!preserveMetabasePlaceholders) {
        ...
        const optionalClausePattern =
          /\s+AND\s+[\w`.-]*url_path\s*=\s*\[\[\s*\{\{\s*url_(?:sti|path)\s*\}\}\s*--\s*\]\]\s*'\/'/gi
  ...
  const prepareLineChartData = (includeAverage: boolean = true): ILineChartProps | null => {
    ...
    if (keys.length >= 3) {
      const isNumericColumn = (key: string): boolean =>
        data.every((row: Record<string, unknown>) => {
          const value = row[key]
      ...
      const seriesMap = new Map<
        string,
        Array<{ x: Date | number; y: number; xAxisCalloutData: string; yAxisCalloutData: string }>
      >()
      ...
      if (includeAverage) {
        ...
        const averagePoints = Array.from(allXValues)
          ...
          .map((xVal) => {
            ...
            lineChartData.forEach((series) => {
              const point = (series.data as Array<{ x: Date | number; y: number; xAxisCalloutData?: string }>).find(
                (p) => {
            ...
            const originalPoint = (
              lineChartData[0]?.data as Array<{ x: Date | number; xAxisCalloutData?: string }> | undefined
            )?.find((p) => {
              const pxVal = p.x instanceof Date ? p.x.getTime() : Number(p.x)
              return pxVal === xVal
            })
  ...
  return (
    <>
                  ...
                  <div className="space-y-3">
                      ...
                      <Alert variant="warning" size="small">
                        <div>
                          ...
                          <p className="mt-1 text-sm">
                            Du bruker både interaktivt datofilter og besøksvarighet, som ikke fungerer sammen i
                            Metabase.
                          </p>
                        </div>
                      </Alert>
                    ...
                    <div className="flex flex-col gap-3">
                      <div>
                        ...
                        {estimating && (
                          <div className="mt-2 text-sm text-[var(--ax-text-subtle)]">Estimerer kostnad...</div>
                        )}
      ...
      >
        ...
        <Modal.Footer>
          <Button variant="primary" onClick={handleConfirmQuery} loading={loading}>
            Ja, kjør spørringen
          </Button>
