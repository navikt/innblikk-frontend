...
export const useProjectManager = () => {
    ...
    (
      ...
      (async () => {
        ...
        try {
          ...
          const sqlCandidates = Array.from(
            new Set([normalizedSqlText, compactSqlForStorage(normalizedSqlText)]),
          ).filter(Boolean)
  ...
  const copyChart = useCallback(
    async (params: {
      ...
      setMessage(null)
      try {
        const sourceGraphs = await api.fetchGraphs(
        ...
        const targetGraphs = await api.fetchGraphs(
  ...
  const editChart = useCallback(
    async (
      ...
      try {
        ...
        const shouldMove =
          (targetDashboardId != null && targetDashboardId !== dashboardId) || targetCategoryId !== categoryId
        if (shouldMove) {
