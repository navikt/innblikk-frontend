...
const markdownToHtml = (markdown: string): string => {
  ...
  const flushParagraph = () => {
    ...
    const content = paragraphBuffer.map((line) => renderInlineMarkdown(line, references)).join('<br />')
...
export const DashboardWidget = ({
  ...
  const canShowActions =
    Boolean(chart.sql) || Boolean(onDeleteChart) || Boolean(onMoveChart) || Boolean(onEditChart) || Boolean(onCopyChart)
  ...
  const tableTotalValue =
    chart.showTotal && chart.type === 'table' && data.length > 0
      ? (() => {
  ...
  const handleCopyShareLink = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        generateShareUrl(chart, websiteId, filters, selectedWebsite?.domain, dashboardTitle),
      )
    ...
    <ActionMenu>
      ...
      <ActionMenu.Content align="end">
        {onEditChart && chart.type === 'text' && (
          <ActionMenu.Item onClick={() => onEditChart(chart.id)}>{`Rediger ${entityLabel}`}</ActionMenu.Item>
        )}
        {!replaceExploreActionWithSqlEditor && (
          <ActionMenu.Item onClick={handleOpenSharedView}>Utforsk grafen</ActionMenu.Item>
        )}
        {showShareAction && (
        ...
        {onCopyChart && chart.sql && (
          <ActionMenu.Item onClick={() => onCopyChart(chart.id, websiteId)}>{`Kopier ${entityLabel}`}</ActionMenu.Item>
        )}
        {onEditChart && chart.type !== 'text' && (
          <ActionMenu.Item onClick={() => onEditChart(chart.id)}>{`Rediger ${entityLabel}`}</ActionMenu.Item>
        )}
        ...
        {onDeleteChart && (
          <ActionMenu.Item onClick={() => onDeleteChart(chart.id)}>{`Slett ${entityLabel}`}</ActionMenu.Item>
        )}
        {(Boolean(data && data.length > 0) || (replaceExploreActionWithSqlEditor && Boolean(chart.sql))) && (
          <ActionMenu.Divider />
        )}
        {data && data.length > 0 && (
          <ActionMenu.Item onClick={() => setShowTransferToMetabaseDialog(true)}>Overfør til Metabase</ActionMenu.Item>
        )}
        {replaceExploreActionWithSqlEditor && chart.sql && (
          <ActionMenu.Item onClick={handleOpenInSqlEditor}>Åpne i SQL-editor</ActionMenu.Item>
        )}
        {data && data.length > 0 && <ActionMenu.Item onClick={handleDownloadCsv}>Last ned CSV</ActionMenu.Item>}
      </ActionMenu.Content>
    </ActionMenu>
  ) : null
        ...
        <div className="flex flex-col mb-4">
          {chart.type === 'table' ? (
            <TableSectionHeader
              ...
              meta={
                tableTotalValue !== null ? (
                  <p className="text-lg text-[var(--ax-text-default)] mt-1">
                    {tableTotalValue.toLocaleString('nb-NO')}{' '}
                    {filters.metricType === 'pageviews'
                      ? 'sidevisninger totalt'
                      : filters.metricType === 'visits'
                        ? 'økter totalt'
                        : 'besøk totalt'}
                  </p>
                ) : undefined
              ...
              actions={chartActions}
              controls={titleBelow ? <div className="mt-2">{titleBelow}</div> : undefined}
            />
          ) : (
            <>
              ...
              {titleBelow && <div className="mt-2">{titleBelow}</div>}
              {tableTotalValue !== null && (
                <p className="text-lg text-[var(--ax-text-default)] mt-1">
                  {tableTotalValue.toLocaleString('nb-NO')}{' '}
                  {filters.metricType === 'pageviews'
              ...
              {chart.description && <p className="text-[var(--ax-text-subtle)] text-sm mt-1">{chart.description}</p>}
            </>
          )}
        </div>
