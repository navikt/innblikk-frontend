...
const JourneyVisualView = ({
  ...
  if (journeys.length === 0) {
    return <div className="text-center text-gray-500 py-8">Ingen forløp matcher søket ditt.</div>
  }
                ...
                {journey.path.map((step, stepIndex) => {
                  ...
                  return (
                    <div key={`${step}-${stepIndex}`} className="flex items-center gap-2">
                      <div
                        className={`w-[320px] min-h-[120px] rounded-lg border bg-[var(--ax-bg-neutral-soft)] p-3 ${isSelectedForFunnel ? 'border-green-500 ring-1 ring-green-500' : 'border-[var(--ax-border-neutral-subtle)]'}`}
                      >
                        <div className="flex items-start justify-between gap-2">
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              onToggleFunnelStep(stepKey, parsedStep.eventName, stepIndex + 1, parsedStep.details)
                            }
