    ...
    >
      <div className="flex flex-col xl:flex-row gap-8 items-start relative">
            ...
            <div className="bg-[var(--ax-bg-neutral-soft)] p-4 rounded-lg border border-[var(--ax-border-neutral-subtle)]">
              ...
              <div className="space-y-3">
                {steps.map((step, index) => (
                  <div
                    key={index}
                    className="border border-gray-300 rounded-lg p-3 bg-[var(--ax-bg-default)] relative shadow-sm"
                  >
                      ...
                      <div className="flex-grow space-y-3">
                        ...
                        {step.type === 'event' && (
                          <RadioGroup
                            ...
                            onChange={(val: string) =>
                              setSteps(updateStepEventScope(steps, index, val as 'current-path' | 'anywhere'))
                            }
                          >
                            ...
                            {step.params && step.params.length > 0 && (
                              <div className="space-y-3 mb-3">
                                {step.params.map((param, pIndex) => (
                                  <div
                                    key={pIndex}
                                    className="bg-[var(--ax-bg-neutral-soft)] rounded-md p-3 relative group border border-[var(--ax-border-neutral-subtle)]"
                                  >
                                    <Button
                                      variant="tertiary-neutral"
                                      size="small"
                                    ...
                                    <div className="flex items-end gap-2 pr-8">
                                      <div className="flex-1">
                                        <TextField
                                          ...
                                          value={param.key}
                                          onChange={(e) =>
                                            setSteps(updateStepParam(steps, index, pIndex, 'key', e.target.value))
                                          }
                                        />
                                      </div>
                                      <div className="pb-1">
                                        <button
                                          ...
                                          onClick={() =>
                                            setSteps(
                                              updateStepParam(
                                                ...
                                                pIndex,
                                                'operator',
                                                param.operator === 'equals' ? 'contains' : 'equals',
                                              ),
                                            )
                                          }
                                          ...
                                          title={
                                            param.operator === 'equals'
                                              ? 'Eksakt match (klikk for inneholder)'
                                              : 'Inneholder (klikk for eksakt)'
                                          }
                                        >
                                          {param.operator === 'equals' ? '=' : '≈'}
                                        </button>
                                    ...
                                    <div className="mt-2">
                                      <TextField
                                        ...
                                        value={param.value}
                                        onChange={(e) =>
                                          setSteps(updateStepParam(steps, index, pIndex, 'value', e.target.value))
                                        }
                                      />
                                    </div>
                                  </div>
                    ...
                    {index < steps.length - 1 && (
                      <div
                        className="absolute left-[23px] top-[40px] w-0.5 h-[calc(100%-10px)] bg-gray-300 -bottom-3 translate-y-full z-0"
                        style={{ height: '24px' }}
                      />
                    )}
                  </div>
        ...
        <div className="flex-1 min-w-0 w-full transition-all duration-300">
            ...
            <>
              ...
              <Tabs value={activeTab} onChange={setActiveTab}>
                ...
                <Tabs.Panel value="vertical" className="pt-4">
                  <FunnelChart data={funnelData} loading={loading} websiteId={selectedWebsite?.id} period={period} />
                        ...
                        actions={
                          <ActionMenu>
                            ...
                            <ActionMenu.Content align="end">
                              ...
                              {funnelSql && (
                                <ActionMenu.Item onClick={() => setModalSql(funnelSql)}>Vis SQL</ActionMenu.Item>
                              )}
                              {funnelQueryStats && (
                                <>
                                  <ActionMenu.Divider />
                      ...
                      <table className="min-w-full divide-y divide-[var(--ax-border-neutral-subtle)]">
                        ...
                        <tbody className="bg-[var(--ax-bg-default)] divide-y divide-[var(--ax-border-neutral-subtle)]">
                          {funnelData.map((item, index) => {
                            ...
                            const percentageOfNext =
                              nextItem && item.count > 0 ? Math.round((nextItem.count / item.count) * 100) : null
                              ...
                              <tr key={index} className="hover:bg-[var(--ax-bg-neutral-soft)]">
                                ...
                                <td className="px-6 py-4 text-base break-all">
                                  {(() => {
                                    ...
                                    if (step?.type === 'event') {
                                      const lenketekst = step.params?.find(
                                        (p) => p.key.toLowerCase() === 'lenketekst',
                                      )?.value
                                      const destinasjon = step.params?.find(
                                        (p) => p.key === 'destinasjon' || p.key === 'url',
                                      )?.value
                                      const tekst = step.params?.find((p) => p.key.toLowerCase() === 'tekst')?.value

                                      return (
                                        <div className="flex flex-col gap-1">
                                          <div className="font-semibold text-[var(--ax-text-default)]">
                                            {step.value}
                                          </div>
                                          {(lenketekst || tekst) && (
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-base">
                                  {dropoffCount !== null && dropoffCount > 0 ? (
                                    <span className="text-red-700 font-medium">
                                      {dropoffPercentage}%{' '}
                                      <span className="font-normal">(-{dropoffCount.toLocaleString('nb-NO')})</span>
                                    </span>
                                  ) : (
                                    '-'
                                  )}
                  ...
                  <>
                    {!showTiming && (
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-gray-500">Kan ta opptil 30 sekunder.</p>
                      </div>
                    )}
                    ...
                    {showTiming &&
                      ...
                      (() => {
                        ...
                        return (
                          <>
                            <div className="mb-4 border border-[var(--ax-border-neutral-subtle)] rounded-lg overflow-hidden bg-[var(--ax-bg-default)]">
                              <div className="p-4 pb-2">
                                <TableSectionHeader
                                  title="Tid per steg og for hele trakten"
                                  actions={
                                    showTiming ? (
                                      <ActionMenu>
                                        <Tooltip content="Flere valg" placement="top">
                            ...
                            {totalTiming && (
                              <div className="mt-2 grid grid-cols-2 gap-4 mb-6">
                                ...
                                <div className="border rounded-lg p-4 bg-[var(--ax-bg-neutral-soft)] border-[var(--ax-border-neutral-subtle)]">
                                  ...
                                  <div className="text-2xl font-bold text-[var(--ax-text-default)]">
                                    {formatDuration(totalTiming.medianSeconds)}
                                  </div>
                                  <div className="text-xs text-[var(--ax-text-subtle)] mt-1">
                                    Median tid fra første til siste steg.
                                  </div>
                                </div>
                              </div>
                            )}

                            <div className="border border-[var(--ax-border-neutral-subtle)] rounded-lg overflow-hidden mb-3 bg-[var(--ax-bg-default)]">
                              <div className="overflow-x-auto px-4">
                                <table className="min-w-full divide-y divide-[var(--ax-border-neutral-subtle)]">
                                  <thead className="bg-[var(--ax-bg-neutral-soft)]">
                                    <tr>
                                      ...
                                      <tr key={index} className="hover:bg-[var(--ax-bg-neutral-soft)]">
                                        <td className="px-6 py-4 text-base text-[var(--ax-text-default)]">
                                          <div className="flex flex-col gap-0.5">
                                            ...
                                            {steps[timing.fromStep]?.type === 'url' &&
                                            timing.fromUrl &&
                                            selectedWebsite ? (
                                              <span
          ...
          <Modal open={!!modalSql} onClose={() => setModalSql(null)} header={{ heading: 'SQL-spørring' }} width={800}>
            <Modal.Body>{modalSql && <SqlViewer sql={modalSql} withoutReadMore showEditButton />}</Modal.Body>
          </Modal>
        </div>
      </div>
    </ChartLayout>
