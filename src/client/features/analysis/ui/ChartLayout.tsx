            ...
            return (
              <li key={page.id}>
                <a
                  ...
                  className={`block px-3 py-2 text-base font-medium rounded-md transition-all duration-200 truncate ${
                    isActive
...
const ChartLayout: React.FC<ChartLayoutProps> = ({
  ...
  const { filteredChartGroups, handleNavigation } = useChartNavigation(
    websiteDomain,
    websiteName,
    websiteId,
      ...
      <AppBlock className="pb-16">
        <div className="rounded-lg shadow-sm border border-[var(--ax-border-neutral-subtle)] mb-8 bg-[var(--ax-bg-default)] overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row min-h-[800px] relative transition-all duration-300">
                ...
                >
                  ...
                  <div className="md:hidden">
                    <Accordion
                      size="small"
                      headingSize="xsmall"
                      className="border-b border-[var(--ax-border-neutral-subtle)] md:border-none"
                    >
                      <Accordion.Item>
                  ...
                  <div
                    className={`hidden md:flex flex-col h-full overflow-y-auto overflow-x-hidden min-w-[250px] p-4 space-y-6 ${isSidebarOpen ? '' : 'md:hidden'}`}
                  >
                    <SidebarNavigationContent
                      filteredChartGroups={filteredChartGroups}
                      currentPage={currentPage}
            ...
            >
              <div className="p-6 md:p-8 flex-1 overflow-x-auto">{children}</div>
            </div>
          </div>
        </div>
      </AppBlock>
