    ...
    <ChartLayout
      ...
      filters={
        <>
          ...
          <RadioGroup size="small" legend="Miljø" value={environment} onChange={(val: string) => setEnvironment(val)}>
      ...
      {!loading && data && (
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <Heading level="2" size="medium">
              Resultater for {getEnvironmentTitle(environment)}
            </Heading>
          </div>
              ...
              <Table sort={sort} onSortChange={handleSort}>
                <Table.Header>
                  <Table.Row>
                    ...
                    <Table.HeaderCell
                      onClick={() => handleSort('custom_events')}
                      align="right"
                      className="cursor-pointer hover:bg-[var(--ax-bg-neutral-soft)]"
                    >
                      Egendefinerte
                    ...
                    {activeTab !== 'attention' && (
                      <Table.HeaderCell
                        onClick={() => handleSort('last_event_at')}
                        align="right"
                        className="cursor-pointer hover:bg-[var(--ax-bg-neutral-soft)]"
                      >
                        Siste aktivitet
                  ...
                  {sortedData.map((row) => (
                    <Table.Row key={row.website_id}>
                      ...
                      <Table.DataCell className="font-medium">
                        <button onClick={() => handleExplore(row)} className="text-blue-600 hover:underline text-left">
                          {row.website_name || row.website_id}
                        </button>
                        {row.domain && <div className="text-xs text-gray-500">{row.domain}</div>}
                      </Table.DataCell>
                      <Table.DataCell align="right">{row.pageviews.toLocaleString('no-NO')}</Table.DataCell>
                      <Table.DataCell align="right">{row.custom_events.toLocaleString('no-NO')}</Table.DataCell>
