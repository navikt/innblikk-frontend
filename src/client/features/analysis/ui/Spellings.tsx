            ...
            <Table.Body>
              {filteredItems.map((item, idx) => (
                <Table.Row key={item.id || idx}>
                  <Table.DataCell className="font-medium text-red-600">{item.word}</Table.DataCell>
                </Table.Row>
              ))}
            </Table.Body>
          </Table>
                      ...
                      <div className="flex flex-col gap-2 min-w-[200px]">
                        <div>
                          ...
                          <div className="text-sm">
                            {crawlInfo?.last_crawl
                              ? new Date(crawlInfo.last_crawl).toLocaleString('nb-NO')
                              : new Date(overviewData.check_date).toLocaleString('nb-NO')}
                          </div>
                        </div>
                        <div>
          ...
          {urlPath && pageId && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div className="bg-[var(--ax-bg-default)] p-4 rounded-lg border border-[var(--ax-border-neutral-subtle)] shadow-sm">
                  <div className="text-sm text-[var(--ax-text-default)] font-medium mb-1">Mulige stavefeil</div>
                  <div className="text-2xl font-bold text-[var(--ax-text-default)]">{potentialMisspellings.length}</div>
                </div>
                <div className="bg-[var(--ax-bg-default)] p-4 rounded-lg border border-[var(--ax-border-neutral-subtle)] shadow-sm">
                  <div className="text-sm text-[var(--ax-text-default)] font-medium mb-1">Bekreftede stavefeil</div>
                  <div className="text-2xl font-bold text-[var(--ax-text-default)]">{misspellings.length}</div>
                </div>
                {overviewData && (
                  <div className="bg-[var(--ax-bg-default)] p-4 rounded-lg border border-[var(--ax-border-neutral-subtle)] shadow-sm">
                    <div className="text-sm text-[var(--ax-text-default)] font-medium mb-1">Siste sjekk</div>
              ...
              <Tabs value={activeTab} onChange={setActiveTab}>
                ...
                <Tabs.Panel value="misspellings" className="pt-4">
                  {renderTable(
                    misspellings,
                    misspellingsSearch
                      ? `Ingen treff for "${misspellingsSearch}"`
                      : 'Ingen bekreftede stavefeil funnet!',
