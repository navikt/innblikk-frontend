...
const BrokenLinks = () => {
  ...
  const { brokenLinks, pagesWithBrokenLinks, siteimproveId, crawlInfo, loading, error, siteimproveBaseUrl } =
    useBrokenLinks(selectedWebsite)

  const filteredPages = pagesWithBrokenLinks.filter(
    (page) => !urlPath || page.url.toLowerCase().includes(urlPath.toLowerCase()),
  )
  const displayedPages = filteredPages.filter((page) =>
    getUrlPath(page.url).toLowerCase().includes(pagesSearch.toLowerCase()),
  )
            ...
            {showPagesQuickStat && (
              <div className="bg-[var(--ax-bg-default)] p-4 rounded-lg border border-[var(--ax-border-neutral-subtle)] shadow-sm">
                <div className="text-sm text-[var(--ax-text-default)] font-medium mb-1">
                  Antall sider med ødelagte lenker
                </div>
            ...
            <div className="pt-4">
              {hasSingleFilteredPage && singleFilteredPage ? (
                <div className="border border-[var(--ax-border-neutral-subtle)] rounded-lg overflow-hidden bg-[var(--ax-bg-default)] p-4">
                  {siteimproveId ? (
                    ...
                    <Alert variant="info" className="mt-4">
                      Kunne ikke hente lenker uten Siteimprove-ID.
                    </Alert>
                  )}
                </div>
                ...
                <Alert variant="success">
                  {urlPath
                    ? pagesSearch
                      ? `Fant ingen treff for "${pagesSearch}" innen "${urlPath}"`
                      : `Fant ingen ødelagte lenker for "${urlPath}"`
                    : pagesSearch
                      ? `Fant ingen treff for "${pagesSearch}"`
