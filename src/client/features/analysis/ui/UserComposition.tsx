...
const UserComposition = () => {
  ...
  const initialPaths =
    pathsFromUrl.length > 0
      ? pathsFromUrl.map((p) => normalizeUrlToPath(p)).filter(Boolean)
      : legacyPath
        ? [normalizeUrlToPath(legacyPath)].filter(Boolean)
        : []
          ...
          <Tabs value={activeCategory} onChange={setActiveCategory}>
            ...
            <div className="mt-6">
              <div className="mb-4 flex flex-wrap justify-start gap-4">
                <Switch checked={showPieChart} onChange={(e) => setShowPieChart(e.target.checked)} size="small">
                  Vis sektordiagram
                </Switch>
                <Switch checked={showBarChart} onChange={(e) => setShowBarChart(e.target.checked)} size="small">
                  Vis stolpediagram
                </Switch>
              </div>
              ...
              {showBarChart && (
                <div className="mt-4 rounded-lg border border-[var(--ax-border-neutral-subtle)] bg-[var(--ax-bg-default)] p-4">
                  {(() => {
                    ...
                    if (!barChartData) {
                      return <Alert variant="info">Kunne ikke lage stolpediagram fra dataene.</Alert>
                    }
                    return (
              ...
              {showPieChart && (
                <div className="mt-4 w-full lg:w-1/2 rounded-lg border border-[var(--ax-border-neutral-subtle)] bg-[var(--ax-bg-default)] p-4">
                  {(() => {
                    ...
                    if (!pieChartData) {
                      return <Alert variant="info">Kunne ikke lage kakediagram fra dataene.</Alert>
                    }
                            ...
                            <div className="mt-2 flex flex-col gap-2">
                              {displayData.map((item, idx) => {
                                ...
                                return (
                                  <div
                                    key={idx}
                                    className="grid grid-cols-[1fr_auto] items-center gap-6 py-1 px-2 hover:bg-[var(--ax-bg-neutral-soft)] rounded"
                                  >
                                    <span>{item.x}</span>
