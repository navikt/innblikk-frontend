...
const OversiktTabContent = ({
  ...
  const handleDownloadCsv = () => {
    ...
    const csvRows = [
      headers.join(','),
      ...processedSeriesData.map((item) => [new Date(item.time).toLocaleString('nb-NO'), item.count].join(',')),
    ]
  ...
  const { chartWrapperRef, dayDividerXs, isMultiDayHourly, formatXAxisDateLabel } = useOversiktDayDividers(
    submittedGranularity,
    submittedDateRange,
    chartData,
    chartKey,
      ...
      {submittedComparePreviousPeriod && comparisonSummary && comparisonRangeLabel && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-[var(--ax-bg-default)] p-4 rounded-lg border border-[var(--ax-border-neutral-subtle)]">
            ...
            <div
              className={`text-2xl font-bold ${comparisonSummary.deltaPercent !== null ? (comparisonSummary.deltaPercent > 0 ? 'text-green-700' : comparisonSummary.deltaPercent < 0 ? 'text-red-700' : 'text-[var(--ax-text-default)]') : 'text-[var(--ax-text-default)]'}`}
            >
        ...
        <div className="flex flex-col gap-4">
          <div className="mb-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex items-center gap-2 sm:ml-auto">
              <Label size="small" htmlFor="traffic-granularity">
                Intervall
              </Label>
              <Select
              ...
              <ActionMenu>
                ...
                <ActionMenu.Content align="end">
                  ...
                  <ActionMenu.Item
                    onClick={() =>
                      openSqlEditorWithContext({ sql: getTrafficSeriesSqlTemplate(), websiteId: selectedWebsite?.id })
                    }
                  >
                    Åpne i SQL-editor
                  </ActionMenu.Item>
