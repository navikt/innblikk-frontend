...
const EditChartDialog = ({
  ...
  const handleSave = async () => {
    ...
    await onSave({
      ...
      sqlText: showSql ? (selectedVariantIsPersisted ? sqlText : (chart.sql ?? '')) : (chart.sql ?? ''),
      ...
      targetQueryId: showSql
        ? selectedVariantIsPersisted
          ? selectedVariant?.queryId
          : undefined
  ...
  useEffect(() => {
    ...
    const selectedVariant =
      nextVariants.find((variant) => variant.queryId === preferredSelectedQueryId) ?? nextVariants[0]
    ...
    const selectedVariantIndex = selectedVariant
      ? nextVariants.findIndex((variant) => variant.queryId === selectedVariant.queryId)
      : -1
  ...
  const handleMoveVariant = async (fromIndex: number, toIndex: number): Promise<boolean> => {
    ...
    setReorderAnnouncement(
      `${getVariantDisplayName(moved.queryName, toIndex)} flyttet til plass ${toIndex + 1} av ${reordered.length}.`,
    )
  ...
  const handleVariantDragStart = (event: DragEvent<HTMLSpanElement>, queryId: number, queryName: string) => {
    ...
    if (index >= 0) {
      setReorderAnnouncement(
        `${getVariantDisplayName(queryName, index)} valgt for flytting. Plass ${index + 1} av ${variants.length}.`,
      )
    }
  }
  ...
  const handleRenameSelectedVariant = async () => {
    ...
    if (selectedVariant.queryId < 0) {
      setVariants((prev) =>
        prev.map((variant) =>
          variant.queryId === selectedVariant.queryId ? { ...variant, queryName: trimmed } : variant,
        ),
      )
    ...
    if (selectedVariant.queryId < 0) {
      setVariants((prev) => {
        ...
        const nextSelectedIndex = nextSelected
          ? next.findIndex((variant) => variant.queryId === nextSelected.queryId)
          : -1
        setRenameVariantValue(
          nextSelected
    ...
    <Modal open={open} onClose={onClose} header={{ heading: 'Rediger graf' }} width="medium">
      <Modal.Body>
        <div className="flex flex-col gap-4">
          ...
          <TextField label="Grafnavn" value={name} onChange={(event) => setName(event.target.value)} size="small" />
          <Select
            label="Graftype"
            value={graphType}
            ...
            <div className="flex flex-col gap-2">
              <div className="flex flex-wrap items-start gap-2">
                <div className="min-w-0">
                  <Tabs size="small" value={String(selectedVariant?.queryId ?? '')} onChange={handleSelectVariant}>
                    <Tabs.List>
                          ...
                          label={
                            <span
                              ...
                              onDragEnter={() => {
                                if (
                                  !canReorderVariants ||
                                  draggedVariantQueryId === null ||
                                  draggedVariantQueryId === variant.queryId
                                )
                                  return
                ...
                {showSql && !showAddVariantControls && (
                  <Button type="button" size="xsmall" variant="secondary" onClick={handleEnableAddVariant}>
                    Legg til variant
                  </Button>
                )}
              </div>
            ...
            <div className="rounded-md border border-[var(--ax-border-neutral-subtle)] p-3">
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <Button type="button" size="xsmall" onClick={handleConfirmAddVariant}>
                    Legg til
                  </Button>
                  <Button type="button" size="xsmall" variant="secondary" onClick={handleCancelAddVariant}>
                    Avbryt
                  </Button>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                <div className="mb-2 flex items-center justify-between gap-2">
                  <label className="block font-medium" htmlFor="edit-chart-sql-editor">
                    SQL-kode
                  </label>
                  ...
                  <Editor
                    ...
                    onChange={(value) => {
                      ...
                      if ((selectedVariant?.queryId ?? 0) < 0) {
                        setVariants((prev) =>
                          prev.map((variant) =>
                            variant.queryId === selectedVariant?.queryId ? { ...variant, sql: nextSql } : variant,
                          ),
                        )
                      }
                    }}
